...
class SVMController extends Controller
{
    ...
    public function index()
    {
        ...
        $filePath = resource_path('json/' . $userName . '/SVM_classification_' . $userName . '.json');
        ...
        return view('svm', ['data' => $jsGridData]);
    }
    ...
    public function visual()
    {
        ...
        $filePath = resource_path('json/' . $namaUser . '/SVM_evaluation_' . $namaUser . '.json'); // Path menuju file JSON di direktori resources

        // Periksa apakah file JSON ada
        if (file_exists($filePath)) {
            // Baca data dari file JSON
            $data = json_decode(file_get_contents($filePath));
            $fileClassification = resource_path('json/' . $namaUser . '/SVM_classification_' . $namaUser . '.json');
